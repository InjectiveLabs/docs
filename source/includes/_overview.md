...
- usually has front-running issues, but those are mitigated at Injective through fast block times and FBA

## Key Differences To CEX

- All information is public which includes even things like untriggered Stop/Take orders.
- The data stored on-chain is minimal for performance reasons and reflects only the current state, relayers provide additional historical data as well as a user interface for traders.

## Comparison to CEX

| Centralized Exchange (CEX) |         Decentralized Exchange (DEX)         |
| :------------------------: | :------------------------------------------: |
|      Exchange Gateway      | Injective Chain Transaction Request Handling |
|  Exchange Matching Engine  |   Injective Chain Decentralized Validators   |
|   Exchange Trade Report    |          Injective Chain EndBlocker          |
|        Co-location         |                Injective Node                |
...
1. Handler: Order Cancellations, Order Creations, Liquidations
2. Market orders matched against old limit orders.
3. New limit orders matched.
